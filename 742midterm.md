...
(a) $\Rightarrow$ (b): Let $S$ be generated by $`\{s_1, \dots, s_n\}`$ as an $R$-module. Multiplication by $x$ is an $R$-endomorphism of $S$. For each $i$, we can write $x s_i = \sum_{j=1}^n a_{ij} s_j$ for some $a_{ij} \in R$. Let $A = (a_{ij})$. The characteristic polynomial $p(t) = \det(tI - A)$ is a monic polynomial in $R[t]$. By the Cayley-Hamilton theorem, $p(x) \cdot 1 = p(x) = 0$, so $x$ is integral over $R$.
