...
- If $a=b^3$ for some $b \in \mathbb{Q}$:
    ...
    - If $a \neq 0$, $p(t)=(t-b)(t^2+bt+b^2)$. The quadratic factor is irreducible over $\mathbb{Q}$ (its discriminant is $-3b^2 < 0$). Thus, there are **two** maximal ideals. To prove they are the only ones, suppose there is a third maximal ideal $\mathfrak{m}$, since $0 = (t-b)(t^2+bt+b^2) \in \mathfrak{m}$, either $t-b \in \mathfrak{m}$ or $t^2+bt+b^2 \in \mathfrak{m}$. In the first case, $\mathfrak{m} \supset (t-b)$, and since $(t-b)$ is maximal, $\mathfrak{m} = (t-b)$. In the second case, $\mathfrak{m} \supset (t^2+bt+b^2)$, and since $(t^2+bt+b^2)$ is maximal, $\mathfrak{m} = (t^2+bt+b^2)$.
