...
Consider an embedding of $S_4$ into $S_5$ as stabilizer of a letter. By primitive element theorem, the corresponding subfield is generated by some element $x$ over $F$. Then $m_x$ has degree 5.

Then the other roots of $m_x$ are obtained by $\operatorname{Gal}(E / F)$ acting on $x$. The stabilizers of the other roots of $m_x$ are conjugates of $S_4$. The intersection of all conjugates of $S_4$ inside $S_5$ is trivial. So the splitting field of $m_x$ is $E$.
